...
import io.sentry.Sentry as ISentry
...
actual object Sentry {
    actual fun init(configuration: (SentryOptions) -> Unit) {
        ...
        SentryAndroid.init(AndroidContext!!) {
            ...
            it.isAttachStacktrace = options.attachStackTrace
            it.isAttachThreads = options.attachThreads
            // it.attachViewHierarchy = options.attachViewHierarchy
            it.isDebug = options.debug
            it.setDiagnosticLevel(options.diagnosticLevel.to())
            it.dsn = options.dsn
            // it.enableAppHangTracking = options.enableAppHangTracking
            it.isEnableAutoSessionTracking = options.enableAutoSessionTracking
            ...
            it.sdkVersion = options.sdk?.toSdkVersion()
